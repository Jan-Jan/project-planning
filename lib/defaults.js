...
const defaultsTasks = config =>
  produce(config, draft => {
    for (const key in draft.tasks) {
      ...
      const commitment = config.staff[refTask.staff].commitment || 1
      const optimistic = Math.ceil((days || 1) / commitment); // a 1 day task finishes on the same day, so 0 has no meaning
      const pessimistic = Math.ceil(((days || 1) + (fat || 0)) / commitment)
      draft.tasks[key] = {
        ...refTask,
